...
int MyFit()
{
  ...
  RooRealVar crySpec("crySpec", "E_{#gamma} [MeV]", 4.0, 7.2);
  ...
  chSpec.plotOn(chFrame, MarkerColor(kBlue), LineColor(kBlue), MarkerSize(0.5), Name("chSpec"));
  ...
  double chiSq = chFrame->chiSquare(11);

  fitFun.plotOn(chFrame, Components(fullErg), LineColor(kMagenta), LineStyle(5));
  fitFun.plotOn(chFrame, Components(firsErg), LineColor(kGreen), LineStyle(5));
  fitFun.plotOn(chFrame, Components(secdErg), LineColor(kCyan+2), LineStyle(5));
  fitFun.plotOn(chFrame, Components(comPdf), LineColor(kOrange+2), LineStyle(5));
  double fpeak, fsigma;
  double dpeak, dsigma;
  ...
  TPaveLabel *pchi2 = new TPaveLabel(0.70, 0.70, 0.85, 0.80, Form("#chi^{2}/ndf = %4.2f", chiSq), "brNDC");
  pchi2 -> SetFillStyle(0);
  pchi2 -> SetBorderSize(0);
  pchi2 -> SetTextSize(0.45);
  pchi2 -> SetTextColor(kBlue);
  pchi2 -> SetFillColor(kWhite);
  chFrame -> addObject(pchi2);
  TPaveLabel *fpk = new TPaveLabel(0.70, 0.85, 0.85, 0.95, Form("peak = %.2f#pm%.2f", fpeak, dpeak), "brNDC");
  fpk -> SetFillStyle(0);
  fpk -> SetBorderSize(0);
  fpk -> SetTextSize(0.45);
  fpk -> SetTextColor(kBlue);
  fpk -> SetFillColor(kWhite);
  chFrame -> addObject(fpk);
  TPaveLabel *fsg = new TPaveLabel(0.70, 0.775, 0.85, 0.875, Form("sigma = %.2f#pm%.2f", fsigma, dsigma), "brNDC");
  fsg -> SetFillStyle(0);
  fsg -> SetBorderSize(0);
  fsg -> SetTextSize(0.45);
  fsg -> SetTextColor(kBlue);
  fsg -> SetFillColor(kWhite);
  chFrame -> addObject(fsg);
  std::cout << "chi2: " << chiSq << "; Probability: " << Prob(chiSq, 151) << std::endl;

  chFrame -> SetYTitle("#Events / (25 keV)");
  chFrame -> Draw();
  can -> SaveAs(oName);
